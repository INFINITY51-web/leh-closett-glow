...
export function AdminSuppliers() {
  ...
  return <div className="mt-8 space-y-6">
    ...
    {open && <form onSubmit={save} className="space-y-6 rounded-xl border border-primary/40 bg-card p-5">
      ...
      <section className="space-y-4 border-b border-border pb-6"><h4 className="text-lg font-semibold">1. Dados do fornecedor</h4><div className="grid gap-4 md:grid-cols-2"><Field label="Nome do fornecedor"><input required value={form.name} onChange={(e) => update("name", e.target.value)} className={inputClass} /></Field><Field label="País"><input value={form.country || ""} onChange={(e) => update("country", e.target.value)} className={inputClass} /></Field><Field label="Identificação interna"><input value={form.internal_code || ""} onChange={(e) => update("internal_code", e.target.value)} className={inputClass} /></Field><label className="flex items-center gap-2 self-end pb-2 text-sm font-medium"><input type="checkbox" checked={form.is_active} onChange={(e) => update("is_active", e.target.checked)} />Fornecedor ativo</label></div></section>
