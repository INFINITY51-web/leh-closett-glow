...
function AppearanceArea({ area, onAreaChange }: { area: string; onAreaChange: (area: string) => void }) {
  ...
  return <div className="mt-8 space-y-6">
    ...
    {area === "Banners" ? <BannersEditor /> : area === "Textos" ? <TextAppearanceEditor /> : area === "Seções" ? <HomeSectionsEditor /> : <section className="rounded-xl border border-border bg-card p-8"><h2 className="text-xl font-semibold">{area}</h2><p className="mt-2 text-sm text-muted-foreground">Esta área está preparada para receber os dados persistidos do Supabase quando o recurso correspondente estiver disponível.</p></section>}
  </div>;
}

type HomeSection = { id: string; title: string; type: "Categoria" | "Destaques" | "Mais vendidos" | "Produtos"; active: boolean; sort_order: number; category_id?: string; product_ids?: string[]; product_sort?: "recent" | "price_desc" | "price_asc" }; 
