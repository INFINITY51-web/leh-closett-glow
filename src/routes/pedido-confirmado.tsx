...
export const Route = createFileRoute("/pedido-confirmado")({
  beforeLoad: async () => {
    if (!supabase || !(await supabase.auth.getSession()).data.session) {
      sessionStorage.setItem("leh-checkout-return", "/pedido-confirmado");
...
type Result = "approved" | "pending" | "rejected" | "cancelled" | "loading" | "error";
...
function OrderConfirmationPage() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      ...
      try {
        ...
        setResult(payment.includes("approved") || payment.includes("paid") ? "approved" : payment.includes("cancel") ? "cancelled" : payment.includes("rejected") ? "rejected" : "pending");
  ...
  const copy = result === "approved" ? ["Pagamento aprovado", "Seu pedido foi confirmado e seguirá para preparação."] : result === "cancelled" ? ["Pagamento cancelado", "O pagamento foi cancelado. O pedido foi preservado."] : result === "rejected" ? ["Pagamento recusado", "O pagamento não foi aprovado. O pedido foi preservado."] : result === "pending" ? ["Pagamento pendente", "Estamos aguardando a confirmação do Mercado Pago."] : result === "error" ? ["Não foi possível consultar", message] : ["Verificando pagamento", message];
